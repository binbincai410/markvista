...
- 每个 Mermaid 图外包一层容器，集成 **svg-pan-zoom**（或等效库）  
- **滚轮缩放**：最小 0.5，最大 10，保持 SVG 清晰  
- **拖拽平移**：鼠标按住拖动可平移视图  
- **默认**：初始自动适配宽度、居中显示  
...
- **布局**：顶部一排按钮 [选择文件] [刷新] [导出 PDF] [导出 PNG]，下方为 Markdown 渲染区域  
- **风格**：白色背景、最大宽度 900px、居中、无侧栏、无深色模式、无复杂动画  
...
若有等效替代（如其他 pan-zoom 库），会保持「SVG、可缩放可平移、整篇导出」的同等效果。
