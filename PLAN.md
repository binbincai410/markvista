...
- 每个 Mermaid 图提供 **「全屏」按钮**（如图角或图下方的图标/文字）  
- 点击后该图进入**全屏层**（遮罩或新层覆盖整个视口）  
- 全屏内支持：**滚轮缩放**（如最小 0.5、最大 10）、**拖拽平移**  
- 全屏内提供关闭方式（如关闭按钮、ESC、点击空白处），退出后回到正文  
- 技术实现：全屏层内使用 **svg-pan-zoom**（或等效库）操作同一张 SVG，保持清晰  
...
- [ ] 至少两个 Mermaid 图能正确显示  
- [ ] 正文内 Mermaid 图完整显示、无缩放/拖拽；每个图有全屏按钮；全屏内可缩放、可拖拽  
- [ ] 导出 PDF 正常、整篇、含图  
- [ ] 导出 PNG 为完整长图、2x、清晰  
- [ ] 右侧章节导航可点击并跳转到对应标题  
